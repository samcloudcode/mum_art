-- Migration 012: assistant edition size and framing updates
-- Dependencies: 011_add_assistant_sales_and_undo.sql.
-- Compatibility: apply before deploying code that proposes size or frame_type.
-- Replaces the bounded apply function with a backwards-compatible extension;
-- no table shape or existing proposal contract changes.
-- Production dry run: 2026-09-01, all 5 migration statements succeeded and
-- rolled back. A second rollback-only transaction applied a combined printed,
-- Large, Framed proposal, verified its three audit rows, undid it exactly, and
-- verified the three undo audit rows.
-- Production applied: 2026-09-01, all 5 statements committed. Post-apply
-- verification passed for function security, fixed search path, role grants,
-- physical-field whitelisting, and physical-field audit paths.
...
'Atomically validates and applies authenticated inventory proposals, including exact sales, physical details, and captured-state undo.';
