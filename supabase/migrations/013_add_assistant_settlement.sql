-- Migration 013: assistant settlement proposals
-- Dependencies: 012_add_assistant_physical_details.sql.
-- Compatibility: apply before deploying code that proposes mark_settled.
-- Replaces the bounded apply function with a backwards-compatible extension;
-- no table shape or existing proposal contract changes. Settlement is limited
-- to setting is_settled on an already-sold, unsettled edition. Safe undo may
-- restore that edition to unsettled without changing its sale details.
-- Local verification: 2026-09-03, applied dependencies through 012 and this
-- migration to PostgreSQL 15, then verified settlement and undo audit rows plus
-- rejection of extra settlement fields, unsold settlement, and other sold-row
-- edits. Production dry run: 2026-09-03, all 5 migration statements succeeded
-- and rolled back. Production applied: 2026-09-03, all 5 statements committed.
-- Post-apply verification passed for the settlement guards, existing physical
-- details and undo paths, security mode, fixed search path, and role grants.
...
BEGIN
    ...
    BEGIN
        ...
        LOOP
            ...
            IF COALESCE(jsonb_typeof(patch) <> 'object', true) OR patch = '{}'::jsonb
               OR EXISTS (
                   ...
                   WHERE key NOT IN (
                       ...
                       'date_in_gallery', 'is_stock_checked', 'size', 'frame_type'
                   )
               )
               OR COALESCE(change->>'action' NOT IN ('update', 'move', 'sell', 'settle', 'undo'), true)
            ...
            IF change->>'action' = 'settle'
               AND NOT (
                   (SELECT count(*) FROM jsonb_object_keys(patch)) = 1
                   AND patch ? 'is_settled'
                   AND jsonb_typeof(patch->'is_settled') = 'boolean'
                   AND (patch->>'is_settled')::BOOLEAN IS TRUE
               ) THEN
                RAISE EXCEPTION 'A settlement change must only mark edition % as settled', eid;
            END IF;
            IF change->>'action' <> 'sell'
               AND patch ? 'is_sold'
               AND jsonb_typeof(patch->'is_sold') = 'boolean'
                       ...
                       OR (
                           ...
                           AND (
                               ...
                               OR (
                                   ...
                                   AND patch ? 'is_settled'
                                   AND jsonb_typeof(patch->'is_settled') = 'boolean'
                                   AND (patch->>'is_settled')::BOOLEAN IS FALSE
                               )
                           )
                       )
                   )
            ...
            END IF;
            IF change->>'action' = 'settle'
               AND (before_row.is_sold IS NOT TRUE OR before_row.is_settled IS TRUE) THEN
                RAISE EXCEPTION 'Edition % must be sold and unsettled before settlement', eid;
            END IF;
            ...
            IF patch ? 'is_printed' AND jsonb_typeof(patch->'is_printed') <> 'boolean'
               ...
               OR patch ? 'distributor_id' AND jsonb_typeof(patch->'distributor_id') NOT IN ('number', 'null')
               OR patch ? 'size' AND jsonb_typeof(patch->'size') NOT IN ('string', 'null')
               OR patch ? 'frame_type' AND jsonb_typeof(patch->'frame_type') NOT IN ('string', 'null') THEN
            ...
            IF patch ? 'commission_percentage' AND patch->'commission_percentage' <> 'null'::jsonb THEN
                ...
                END IF;
            END IF;
            IF patch ? 'size' AND patch->'size' <> 'null'::jsonb
               AND patch->>'size' NOT IN ('Small', 'Large', 'Extra Large') THEN
                RAISE EXCEPTION 'Invalid size for edition %', eid;
            END IF;
            IF patch ? 'frame_type' AND patch->'frame_type' <> 'null'::jsonb
               AND patch->>'frame_type' NOT IN ('Framed', 'Tube only', 'Mounted') THEN
                RAISE EXCEPTION 'Invalid frame type for edition %', eid;
            END IF;
    ...
    LOOP
        ...
        UPDATE editions SET
            ...
            size = CASE WHEN patch ? 'size'
                THEN CASE WHEN patch->'size' = 'null'::jsonb
                    THEN NULL ELSE patch->>'size' END
                ELSE size END,
            frame_type = CASE WHEN patch ? 'frame_type'
                THEN CASE WHEN patch->'frame_type' = 'null'::jsonb
                    THEN NULL ELSE patch->>'frame_type' END
                ELSE frame_type END,
            updated_at = now()
        ...
        END IF;
        IF before_row.size IS DISTINCT FROM after_row.size THEN
            INSERT INTO activity_log(user_id,user_email,action,entity_type,entity_id,entity_name,field_name,old_value,new_value,description,proposal_id,source)
            VALUES(auth.uid(),jwt_email,change->>'action','edition',eid,after_row.edition_display_name,'size',before_row.size,after_row.size,safe_description,p.id,'assistant');
        END IF;
        IF before_row.frame_type IS DISTINCT FROM after_row.frame_type THEN
            INSERT INTO activity_log(user_id,user_email,action,entity_type,entity_id,entity_name,field_name,old_value,new_value,description,proposal_id,source)
            VALUES(auth.uid(),jwt_email,change->>'action','edition',eid,after_row.edition_display_name,'frame_type',before_row.frame_type,after_row.frame_type,safe_description,p.id,'assistant');
        END IF;
...
'Atomically validates and applies authenticated inventory proposals, including exact sales, settlement, physical details, and captured-state undo.';
