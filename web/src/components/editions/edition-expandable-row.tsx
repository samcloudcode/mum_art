...
export function EditionExpandableRow({ edition, onUpdateNotes, colSpan }: Props) {
  ...
  return (
    <tr className="bg-card border-b-0">
      <td colSpan={colSpan} className="p-0">
        <div className="exhibition-label mx-4 my-3 rounded-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
            {/* Sales Information */}
            <div className="space-y-3">
              <h4 className="font-serif font-medium text-foreground text-sm border-b border-border/50 pb-1">
                Sales Information
              </h4>
              <dl className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Date Sold</dt>
                  <dd className="text-foreground">{edition.date_sold ? formatDate(edition.date_sold) : '-'}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Sale Price</dt>
                  <dd className="text-foreground font-serif tabular-nums">
                ...
                <div className="flex justify-between">
                  ...
                  <dd className="text-foreground font-medium font-serif tabular-nums">
                    {netRevenue !== null ? `£${netRevenue.toLocaleString()}` : '-'}
                  </dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Payment Status</dt>
                  <dd className={edition.is_settled ? 'text-seafoam' : 'text-coral'}>
                ...
                {edition.payment_note && (
                  <div className="flex justify-between">
                    ...
                    <dd className="text-foreground">{edition.payment_note}</dd>
                  </div>
                )}
              </dl>
            </div>
            ...
            <div className="space-y-3">
              ...
              <dl className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <dd className="text-foreground">{edition.distributors?.name || 'Direct'}</dd>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Days at Location</dt>
                  <dd className="text-foreground font-serif tabular-nums">{daysAtLocation !== null ? daysAtLocation : '-'}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Gallery Commission</dt>
                  <dd className="text-foreground">
              ...
              {/* Edition Details */}
              <h4 className="font-serif font-medium text-foreground text-sm border-b border-border/50 pb-1 mt-4">
                Edition Details
              </h4>
              <dl className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Edition Number</dt>
                  <dd className="text-foreground font-serif">{edition.edition_number || '-'}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Variation</dt>
                  <dd className="text-foreground">{edition.variation || '-'}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Stock Checked</dt>
                  <dd className={edition.is_stock_checked ? 'text-seafoam' : 'text-muted-foreground'}>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between border-b border-border/50 pb-1">
                <h4 className="font-serif font-medium text-foreground text-sm">Notes</h4>
                {!isEditingNotes && (
                  <Button
                    variant="ghost"
                    size="sm"
