...
test('agent investigates database facts and uses the proposal as confirmation', () => {
  ...
  assert.match(prompt, /Review unconfirmed stock at Kendalls.*get_gallery_stock with edition details/)
  assert.match(prompt, /Do not propose moving anything until the user identifies it as missing/)
